...
/* this ring sz forces the second message to wrap a byte before end of prefix */
#define ring_sz (sizeof(size_t) + sizeof(msg) + sizeof(size_t) - 1)
...
int main() {
  ...
  issue(W, do_write); delay(); /* msg wraps a byte before end of prefix */
  issue(R, do_read); delay();
