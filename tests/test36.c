  ...
  for(;;) {
    ...
    switch(op) {
      ...
      case do_read:
        ...
        if (rc > 0) printf("r: [%.*s]\n", rc, buf);
        else if (rc == 0) printf("r: wouldblock\n");
        break;
      case do_close:
        assert(s);
        shr_close(s);
...
int main() {
  ...
  issue(W, do_write);/* squirrel0_ (u = 1; we show 0=nul and _ is unused) */
  issue(W, do_write);/* quirrel00s */
  issue(W, do_write);/* uirrel00sq */
  /* read position is lru here ^   */

  issue(R, do_read); /*        0s  */ // shows as empty due to nul terminator
  issue(R, do_read); /* u        q */ // shows as qu (wrap)
  issue(R, do_read); /*  ir        */ 
  issue(R, do_read); /*    re      */
  issue(R, do_read); /*      l0    */
