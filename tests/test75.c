...
#define OPS o(do_none) o(do_open) o(do_fill) o(do_read_half) o(do_read_one) \
 o(do_write_one) o(do_farm_stat) o(do_close) o(do_exit)
#define o(x) #x,
char *op_s[] = { OPS };
#undef o
#define o(x) x,
...
struct events {
  ...
  ops op;
    ...
    { W, do_fill},       /* write seq 0-9 */
    { W, do_write_one},  /* write seq 10 */
    { W, do_write_one},  /* write seq 11 */
    { W, do_write_one},  /* write seq 12 */
    { W, do_write_one},  /* write seq 13 */

    { F, do_open},
    { F, do_read_one},   /* read eldest: seq 4 */
    { R, do_read_one},   /* read eldest: seq 4 */

    { F, do_farm_stat},  /* missed none, because opened after that */
    { R, do_farm_stat},  /* missed 4, since opened before dropped */

    { F, do_read_half},  /* read 5-9 */
    { R, do_read_half},  /* read 5-9 */

    { F, do_read_half},  /* read 10-13, then exhaust */
    { R, do_read_half},  /* read 10-13, then exhaust */
  ...
  for(i=0; i < adim(ev); i++) {
    ...
    switch( ev[i].op ) {
      ...
      case do_write_one:
        ...
        printf("w: wrote 1 message (to seq %u)\n", seq);
        break;
      case do_fill:
        for(n=0; n < NMSG; n++) {
          snprintf(msg_one, sizeof(msg_one), "%u", seq++);
