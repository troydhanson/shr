...
#define OPS o(do_none) o(do_open) o(do_fill) o(do_read_half) o(do_read_one) \
 o(do_write_20b) o(do_write_one) o(do_write_one_2b) o(do_write_one_3b) o(do_write_one_4b) o(do_write_one_5b) o(do_stat) o(do_farm_stat) o(do_close) o(do_exit)
#define o(x) #x,
char *op_s[] = { OPS };
#undef o
#define o(x) x,
...
struct events {
  ...
  ops op;
    ...
    { W, do_write_20b},  /* fill ring in a single message */
    { W, do_write_20b},  /* fill ring in a single message */
    { W, do_write_20b},  /* fill ring in a single message */
    { W, do_stat},       /* mu = 1, md = 2, bu = 20 */

    { R, do_read_one},   /* read cdefghijklmnopqrstuv */

    { W, do_write_20b},  /* fill ring in a single message */
    { W, do_write_one},  /* replace 20b with 1b message */
    { W, do_stat},       /* mu = 1, md = 3, bu = 1 */
    { R, do_read_one},   /* read e */

    { W, do_write_20b},  /* fill ring in a single message */
    { R, do_read_one},   /* read fghijklmnopqrstuvwxy */
    ...
    { W, do_write_20b},  /* fill ring in a single message */
    { F, do_read_one},   /* misses h, reads ijklmnopqrstuvwxyz{| */
  ...
  for(i=0; i < adim(ev); i++) {
    ...
    switch( ev[i].op ) {
      ...
      case do_read_one:
        ...
        break;
      case do_write_20b:
        for(nr=0; nr < 20; nr++) msg_one[nr] = seq+nr;
        nr = shr_write(s, msg_one, 20);
        if (nr == 20) 
          printf("w: wrote %.*s\n", (int)nr, msg_one);
        else
          printf("shr_write: %zd\n", nr);
