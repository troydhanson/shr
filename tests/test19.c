...
#define OPS o(do_none) o(do_open) o(do_write_one) o(do_stat) o(do_read_one) \
...
struct events {
  ...
  ops op;
    ...
    { W, do_stat},      /* confirms in cache */
    { W, do_write_one}, /* writes seq 1 to cache; flushes seq 0 to ring */
    { W, do_close},     /* shr_close will shr_flush(s,0) 
                           but cached seq 1 cannot flush to ring,
                           because there is no room for it */

    { R, do_stat},      /* confirms in ring */
